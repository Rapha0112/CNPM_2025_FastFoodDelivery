  ...
  const fetchFeedbackStatus = async (userId) => {
    try {
      const response = await axios.get(
        `http://localhost:5213/api/FeedBacks/ViewAllFeedBackByUserID/${userId}`
      );
      ...
      onOk: async () => {
        try {
          const response = await axios.delete(
            `http://localhost:5213/api/Orders/GetConfirmOrderByUser/${orderId}`
          );
          ...
          onFinish={async (values) => {
            try {
              const response = await axios.post(
                "http://localhost:5213/api/FeedBacks/CreateFeedBack",
                {
                  userId: UserId,
                  orderId: orderId,
          ...
          onFinish={async (values) => {
            try {
              const response = await axios.put(
                `http://localhost:5213/api/FeedBacks/UpdateFeedBack/${feedbackId}`,
                {
                  commentMsg: values.comment,
                }
              );
  ...
  const handleDeleteFeedback = async (feedbackId, orderId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await axios.delete(
            `http://localhost:5213/api/FeedBacks/DeleteFeedBack/${feedbackId}`
          );
