...
function FoodItemManagement() {
  ...
  const handleDeleteFastFood = async (foodId) => {
    ...
    await axios.delete(
      `http://localhost:5213/api/MenuItemFood/DeleteFood/${foodId}`
    );
  ...
  async function handleSubmit(values) {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5213/api/MenuItemFood/CreateFood",
        values
      );
  ...
  async function handleEditCource() {
    ...
    try {
      ...
      axios
        .put(
          `http://localhost:5213/api/MenuItemFood/UpdateFood/${oldFood.foodId}`,
