...
namespace Business_Layer.Configuration
{
    public class OrderConfiguration : IEntityTypeConfiguration<Order>
    {
        public void Configure(EntityTypeBuilder<Order> builder)
        {
           ...
           // Cấu hình mối quan hệ với Drone
            builder.HasOne(x => x.Drone)
                   .WithMany() // Một Drone giao nhiều đơn, nhưng bên Drone không cần list Orders
                   .HasForeignKey(x => x.DroneId)
                   .OnDelete(DeleteBehavior.SetNull); // Nếu xóa Drone, set DroneId trong Order về null (để giữ lịch sử đơn)
            // ---------------------
        }
    }
}
